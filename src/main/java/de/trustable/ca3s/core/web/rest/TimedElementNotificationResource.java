...
public class TimedElementNotificationResource {
    ...
    /**
     * {@code PATCH  /timed-element-notifications/:id} : Partial updates given fields of an existing timedElementNotification, field will ignore if it is null
     *
     * @param id the id of the timedElementNotification to save.
     * @param timedElementNotification the timedElementNotification to update.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the updated timedElementNotification,
     * or with status {@code 400 (Bad Request)} if the timedElementNotification is not valid,
     * or with status {@code 404 (Not Found)} if the timedElementNotification is not found,
     * or with status {@code 500 (Internal Server Error)} if the timedElementNotification couldn't be updated.
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PatchMapping(value = "/timed-element-notifications/{id}", consumes = { "application/json", "application/merge-patch+json" })
    public ResponseEntity<TimedElementNotification> partialUpdateTimedElementNotification(
        @PathVariable(value = "id", required = false) final Long id,
        @NotNull @RequestBody TimedElementNotification timedElementNotification
    ) throws URISyntaxException {
        log.debug("REST request to partial update TimedElementNotification partially : {}, {}", id, timedElementNotification);
        if (timedElementNotification.getId() == null) {
            throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
        }
        if (!Objects.equals(id, timedElementNotification.getId())) {
        ...
        Optional<TimedElementNotification> result = timedElementNotificationService.partialUpdate(timedElementNotification);

        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, timedElementNotification.getId().toString())
        );
    }
