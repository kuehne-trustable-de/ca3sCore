...
public class ContentUploadProcessor {
	...
	static HashMap<String,ASN1ObjectIdentifier> nameOIDMap = new HashMap<String,ASN1ObjectIdentifier>();
	static HashMap<String,Integer> nameGeneralNameMap = new HashMap<String,Integer>();
	static {
    ...
    public ResponseEntity<PkcsXXData> uploadContent(@Valid @RequestBody UploadPrecheckData uploaded) {
    	...
    	if( CreationMode.COMMANDLINE_TOOL.equals(creationMode)) {
			LOG.debug("not supported creation mode {} ", creationMode);
    		...
    		try{
	    	    ...
	    	    List<GeneralName> gnList = new ArrayList<GeneralName>();
				...
				LOG.debug("problem creating serverside csr: " + ex.getMessage());
	...
	private CSR startCertificateCreationProcess(final String csrAsPem, PkcsXXData p10ReqData, final String requestorName, String requestorComment, Optional<Pipeline> optPipeline )  {
		...
		if( optPipeline.isPresent()) {
			...
			CSR csr = cpUtil.buildCSR(csrAsPem, requestorName, AuditUtil.AUDIT_WEB_CERTIFICATE_REQUESTED, "", pipeline, messageList );
