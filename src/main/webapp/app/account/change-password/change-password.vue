    ...
    <div>
        ...
        <div class="row justify-content-center">
            <div class="col-md-8 toastify-container">
                ...
                <form v-if="!settingsAccount.managedExternally" name="form" role="form" id="password-form" v-on:submit.prevent="changePassword()">
                    ...
                    <div class="form-group">
                        ...
                        <div v-if="$v.resetPassword.currentPassword.$anyDirty && $v.resetPassword.currentPassword.$invalid">
                            <small class="form-text text-danger"
                                   v-if="!$v.resetPassword.currentPassword.required" v-text="$t('global.messages.validate.newpassword.required')"></small>
                        </div>
                    </div>
                    ...
                    <div v-if="credentialChange.credentialUpdateType === 'PASSWORD'" class="form-group">
                        ...
                        <div>
                            ...
                            <small class="form-text text-danger" v-if="showRegExpFieldWarning($v.resetPassword.newPassword.$model, regExpSecret())"
                            v-text="$t('ca3SApp.messages.password.requirement.' + regExpSecretDescription())">
                            </small>
                        </div>
                    ...
                    <button type="submit" :disabled="!canSubmit()" class="btn btn-primary" v-text="$t('password.form.button')"></button>
                </form>

                <div v-if="!settingsAccount.managedExternally">
                  <p></p>
                ...
                <p></p>
                <p></p>
                <h3 id="token-title"><span v-text="$t('global.messages.token.title')"></span></h3>

                <router-link :to="{name: 'EditCredential', params: {credentialType: 'API_TOKEN'}}"
                             tag="button" id="jh-create-entity" class="btn btn-primary float-right jh-create-entity ">
                    <font-awesome-icon icon="plus"></font-awesome-icon>
                    <span v-text="$t('ca3SApp.form.client.token.api.create')"></span>
                </router-link>

                <!--router-link :to="{name: 'EditCredential', params: {credentialType: 'SCEP_TOKEN'}}"
                             tag="button" id="jh-create-entity" class="btn btn-primary float-right jh-create-entity ">
                    <font-awesome-icon icon="plus"></font-awesome-icon>
                    <span v-text="$t('ca3SApp.form.client.token.scep.create')"></span>
                </router-link>

                <router-link :to="{name: 'EditCredential', params: {credentialType: 'EST_TOKEN'}}"
                             tag="button" id="jh-create-entity" class="btn btn-primary float-right jh-create-entity ">
                    <font-awesome-icon icon="plus"></font-awesome-icon>
                    <span v-text="$t('ca3SApp.form.client.token.est.create')"></span>
                </router-link-->

                <router-link :to="{name: 'EditCredential', params: {credentialType: 'EAB_PASSWORD'}}"
                             tag="button" id="jh-create-entity" class="btn btn-primary float-right jh-create-entity ">
                    <font-awesome-icon icon="plus"></font-awesome-icon>
                    <span v-text="$t('ca3SApp.form.client.password.eab.create')"></span>
                </router-link>

                <div class="table-responsive">
                    <table class="table table-striped" id="token-list">
                        <thead>
                        <tr>
                            <th><span v-text="$t('ca3SApp.form.client.auth.type')"></span></th>
                            <th><span v-text="$t('ca3SApp.form.client.auth.requestedOn')"></span></th>
                            <th><span v-text="$t('ca3SApp.form.client.auth.validTo')"></span></th>
                            <th><span v-text="$t('global.menu.config.pipeline')"></span></th>
                            <th><span v-text="$t('ca3SApp.form.client.token.status')"></span></th>
                            <th></th>
                        </tr>
                        </thead>
                        <tbody>
                        <tr v-for="token in tokenArr" :key="token.id">
                            ...
                            <td class="text-right">
                                <div class="btn-group">
                                    <b-button v-on:click="prepareRemove(token)"
                                              variant="danger"
                                              class="btn btn-sm"
                                              v-b-modal.removeEntity>
                                        <font-awesome-icon icon="times"></font-awesome-icon>
        ...
        <b-modal ref="removeEntity" id="removeEntity" >
            ...
            <div slot="modal-footer">
                ...
                <button type="button" class="btn btn-primary" id="jhi-confirm-delete-clientAuthSecret" v-text="$t('entity.action.delete')" v-on:click="removeCredential()"></button>
            </div>
        </b-modal>
    </div>
</template>
