<template>
    <div>
        ...
        <div class="alert alert-warning" v-if="!isFetching && cAConnectorConfigs && cAConnectorConfigs.length === 0">
            <span v-text="$t('ca3SApp.cAConnectorConfig.home.notFound')"></span>
        </div>
        <div class="table-responsive" v-if="cAConnectorConfigs && cAConnectorConfigs.length > 0">
            ...
            <select float="left" class="smallSelector fa-1x" v-model="typeFilter"
                ...
                <option value="VAULT" v-bind:label="$t('ca3SApp.CAConnectorType.VAULT')">VAULT_INVENTORY</option>
            ...
            <table class="table table-striped">
                ...
                <tr>
                    ...
                    <th><span v-text="$t('ca3SApp.cAConnectorConfig.pollingOffset')"></span></th>
                    <th><span v-text="$t('ca3SApp.cAConnectorConfig.defaultCA')"></span></th>
                    <th><span v-text="$t('ca3SApp.cAConnectorConfig.active')"></span></th>
                    <th><span v-text="$t('ca3SApp.cAConnectorConfig.expiresOn')"></span></th>
                    <th><span v-text="$t('ca3SApp.cAConnectorConfig.selector')"></span></th>
                    <th><span v-text="$t('ca3SApp.cAConnectorConfig.interval')"></span></th>
                    <th></th>
                </tr>
                </thead>
                <tbody>
                <tr v-for="cAConnectorConfig in filteredCAConnectorConfigs"
                    ...
                    <td v-else-if="cAConnectorConfig.caConnectorType === 'VAULT'">{{cAConnectorConfig.selector + " / " + cAConnectorConfig.role}}</td>
                    <td v-else></td>
        ...
        <b-modal ref="removeEntity" id="removeEntity" >
            <span slot="modal-title"><span id="ca3SApp.cAConnectorConfig.delete.question" v-text="$t('entity.delete.title')"></span></span>
            <div class="modal-body">
                <p id="jhi-delete-cAConnectorConfig-heading" v-text="$t('ca3SApp.cAConnectorConfig.delete.question', {'id': removeId})"></p>
            </div>
            <div slot="modal-footer">
                <button type="button" class="btn btn-secondary" v-text="$t('entity.action.cancel')" v-on:click="closeDialog()"></button>
                <button type="button" class="btn btn-primary" id="jhi-confirm-delete-cAConnectorConfig" v-text="$t('entity.action.delete')" v-on:click="removeCAConnectorConfig()"></button>
            </div>
        </b-modal>
    </div>
</template>
