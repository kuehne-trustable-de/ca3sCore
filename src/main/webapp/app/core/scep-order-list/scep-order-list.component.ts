...
VuejsDatatableFactory.registerTableType<any, any, any, any, any>('orders-table', tableType =>
  tableType
    ...
    .setDisplayHandler(async ({ source: baseEndPoint, paged: endpointDesc }) => {
      ...
      } as ITableContentParam<IScepOrderView>;
    })
    .mergeSettings({
      table: {
...
export default class CsrList extends mixins(AlertMixin, Vue) {
  ...
  public scepOrderSelectionItems: ICertificateSelectionData[] = [
    {
      itemName: 'status',
      itemType: 'set',
      itemDefaultSelector: 'EQUAL',
      itemDefaultValue: 'VALID',
      values: ['PENDING', 'READY', 'INVALID'],
    },
    ...
    { itemName: 'transId', itemType: 'string', itemDefaultSelector: 'LIKE', itemDefaultValue: '' },
  ...
  public defaultFilter: ICertificateFilter = { attributeName: 'status', attributeValue: 'READY', selector: 'EQUAL' };
  ...
  public getInputType(itemName: string): string {
    const selectionItem = this.scepOrderSelectionItems.find(selections => selections.itemName === itemName);
    if (selectionItem) {
  ...
  data() {
    ...
    return {
      columns: [
        { label: 'id', field: 'id' },
        { label: this.$t('ca3SApp.scepOrder.transId'), field: 'transId' },
        { label: this.$t('status'), field: 'status' },
        { label: this.$t('realm'), field: 'realm' },
        { label: this.$t('ca3SApp.scepOrder.pipeline'), field: 'pipelineName' },
        { label: this.$t('ca3SApp.cSR.requestedOn'), field: 'requestedOn' },
        { label: this.$t('ca3SApp.cSR.requestedBy'), field: 'requestedBy' },
        { label: this.$t('subject'), field: 'subject' },
        { label: this.$t('sans'), field: 'sans' },
      ] as TColumnsDefinition<IScepOrderView>,
      page: 1,
      filter: '',
      contentAccessUrl: '',

      get scepOrderApiUrl() {
        window.console.info('scepOrderApiUrl returns : ' + self.contentAccessUrl);
  ...
  public buildContentAccessUrl() {
    const url = this.buildAccessUrl('api/scepOrderList');
  ...
  public getUsersFilterList(): void {
    ...
    axios({
      method: 'get',
      url: 'api/userProperties/filterList/scepOrderList',
      responseType: 'stream',
